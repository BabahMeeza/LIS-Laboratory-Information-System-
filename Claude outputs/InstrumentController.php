...
final class InstrumentController extends Controller
{
    ...
    public function simpan(array $params = []): Response
    {
        ...
        $data = $this->validasi([
            ...
            'protokol'  => 'required|in:astm,hl7,raw,a15',
            'transport' => 'required|in:tcp_server,tcp_client,serial,file',
        ...
        $simpan = [
            ...
            'host'          => in_array($transport, ['serial', 'file'], true) ? null : ($this->request->str('host') ?: null),
            'port'          => in_array($transport, ['serial', 'file'], true) ? null : ($this->request->int('port') ?: null),
        ...
        // Transport BERKAS (BioSystems A15): tidak ada host/port, yang ada
        // dua jalur folder. Kolomnya hanya disentuh untuk transport ini —
        // alat lain tetap dapat disimpan walau migrasi 10 belum dijalankan.
        if ($transport === 'file') {
            $simpan['folder_keluar']     = $this->request->str('folder_keluar') ?: null;
            $simpan['folder_masuk']      = $this->request->str('folder_masuk') ?: null;
            $simpan['folder_jeda_detik'] = max(2, $this->request->int('folder_jeda_detik', 5));

            if ($simpan['folder_keluar'] === null && $simpan['aktif'] === 1) {
                Flash::error('Folder Keluar wajib diisi sebelum alat berbasis berkas diaktifkan '
                    . '(mis. \\\\192.168.20.x\\A15\\Export).');

                return $this->redirect($id > 0 ? "/alat/$id/edit" : '/alat/baru');
            }
            if ($simpan['mode'] === 'bidirectional' && $simpan['folder_masuk'] === null && $simpan['aktif'] === 1) {
                Flash::error('Mode dua arah menulis worklist ke Folder Masuk — isi foldernya, '
                    . 'atau pilih mode Satu arah.');
        ...
        if (!in_array($transport, ['serial', 'file'], true) && ($simpan['port'] === null || $simpan['port'] <= 0)) {
