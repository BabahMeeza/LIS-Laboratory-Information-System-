...
final class ResultProcessor
{
    ...
    private static function cariKonteks(string $sampleId): ?array
    {
        ...
        // 2. Nomor lab, nomor order, atau nomor order Khanza — pencocokan
        //    order saja.
        //
        //    Nomor Khanza perlu ikut dicari: barcode tabung memakai nomor
        //    Khanza HANYA bila nomor itu belum terpakai. Order Khanza yang
        //    diimpor ulang (mis. setelah yang pertama dibatalkan) jatuh ke
        //    barcode LIS, padahal petugas tetap memasukkan nomor Khanza di
        //    alat — dan tanpa baris ini hasilnya "tidak cocok" meski
        //    ordernya ada.
        $row = self::konteksTanpaSpesimen(
            'o.no_lab = ? OR o.no_order = ? OR o.khanza_noorder = ?',
    ...
    private static function pemetaan(int $instrumentId, string $kodeAlat): ?array
    {
        ...
        //
        // DIBATASI PADA KATEGORI ALAT.
        //
        // Tanpa pembatas ini, kode "WBC" dari alat urinalisa akan menemukan
        // pemeriksaan WBC hematologi — Leukosit darah — dan hasil hitung
        // sedimen urine tersimpan sebagai hitung leukosit darah. Angkanya
        // tetap tampak wajar, nilai rujukan yang dipakai ikut yang darah,
        // dan tidak ada tanda apa pun bahwa sesuatu salah. Kesalahan
        // semacam ini sampai ke klinisi.
        //
        // Bila alat belum berkategori, jalan pintas ini tidak dipakai sama
        // sekali: kodenya dicatat sebagai parameter baru yang menunggu
        // keputusan petugas. Lebih baik satu hasil tertahan daripada satu
        // hasil masuk ke pemeriksaan yang keliru.
        $alat = Database::selectOne(
            'SELECT category_id FROM instruments WHERE id = ? LIMIT 1',
            [$instrumentId]
        );
        $kategoriAlat = ($alat['category_id'] ?? null) === null ? 0 : (int) $alat['category_id'];
        ...
        $test = Database::selectOne(
            'SELECT id, kode, nama FROM tests WHERE kode = ? AND aktif = 1 AND category_id = ? LIMIT 1',
            [$kodeAlat, $kategoriAlat]
        );
    ...
    private static function cariOrderItem(array $konteks, int $testId): ?array
    {
        return Database::selectOne(
            'SELECT oi.*, t.tipe_hasil, t.pilihan, t.desimal, t.satuan, t.satuan_ucum,
    ...
    private static function simpanHasil(
        ...
        // Pemeriksaan bertipe pilihan: selaraskan ejaan alat ("-", "+1",
        // "+-") dengan pilihan master ("Negatif", "1+", "Trace"). Harus
        // SEBELUM uji angka di bawah — "+1" lolos is_numeric() dan akan
        // tersimpan sebagai angka 1. Lihat PilihanHasil.
        if (($item['tipe_hasil'] ?? '') === 'pilihan') {
            try {
                $nilaiMentah = PilihanHasil::cocokkan($nilaiMentah, PilihanHasil::daftar($item['pilihan'] ?? null));
            } catch (\Throwable $e) {
                // Nilai asli alat tetap disimpan; petugas memilih manual.
                Logger::error('Penyelaras pilihan tidak dapat dimuat — nilai alat disimpan apa adanya. '
                    . 'Periksa app/Services/PilihanHasil.php (ada? terbaca oleh Apache?).', ['galat' => $e->getMessage()]);
            }
        }
        ...
        // Umur tidak diketahui pada pemeriksaan yang rujukannya bertingkat
        // menurut umur. Nilainya tetap disimpan — pemeriksaannya memang
        // terjadi — tetapi tanpa flag, dan tidak boleh lolos otomatis.
        $umurTidakPasti = $umur === null && $ruj === null;

        if ($umurTidakPasti) {
            Logger::warning(
                'Tanggal lahir pasien belum tercatat — penilaian flag ditahan '
                . 'karena rujukan pemeriksaan ini bertingkat menurut umur.',
                [
                    'test'      => $item['kode_test'] ?? $testId,
                    'order'     => $konteks['no_order'] ?? '',
                    'patient'   => $konteks['patient_id'] ?? null,
                ]
            );
        }
        ...
        if (is_numeric($normal) && ($item['tipe_hasil'] ?? '') !== 'pilihan') {
        ...
        // ----------------------------------------------------------------
        // Keadaan mutu yang berlaku atas nilai ini.
        //
        // Dua hal yang sebelumnya tidak pernah ditanyakan sebelum hasil
        // keluar: bagaimana kontrol kualitas alat pada saat itu, dan
        // apakah lot reagennya masih sah. Keduanya kini ikut menentukan.
        // Lihat QcGateService untuk alasan lengkapnya.
        // ----------------------------------------------------------------
        //
        // Bila modul mutu itu sendiri tidak dapat dimuat (berkas hilang,
        // izin baca salah, tabel belum dimigrasi), hasil pasien TETAP
        // disimpan — hasil yang tidak tampil sama sekali lebih berbahaya
        // daripada hasil tanpa penilaian QC. Sebagai gantinya autovalidasi
        // dimatikan untuk hasil ini, sehingga setiap hasil melewati mata
        // manusia sampai modulnya diperbaiki.
        $lotId = isset($konteks['reagent_lot_id']) && $konteks['reagent_lot_id'] !== null
            ? (int) $konteks['reagent_lot_id'] : null;

        try {
            $qc  = QcGateService::nilai($testId, $instrumentId);
            $lot = QcGateService::periksaLot($lotId);
        } catch (\Throwable $e) {
            Logger::error('Modul kontrol kualitas tidak dapat dijalankan — hasil tetap disimpan, '
                . 'autovalidasi dimatikan. Periksa app/Services/QcGateService.php (ada? terbaca oleh Apache?).', [
                'galat' => $e->getMessage(),
                'test'  => $item['kode_test'] ?? $testId,
            ]);
            $qc = [
                ...
                'menahan'      => false,
                'boleh_auto'   => false,
                'alasan'       => 'Modul QC tidak dapat dimuat.',
                'qc_result_id' => null,
                'tgl_uji'      => null,
                'westgard'     => null,
                'umur_jam'     => null,
            ];
            $lot        = ['sah' => false, 'alasan' => 'Modul QC tidak dapat dimuat.'];
            $autoVerify = false;
        }

        // ----------------------------------------------------------------
        // Keputusan autovalidasi.
        //
        // Aturannya tidak lagi ditulis di sini melainkan dibaca dari
        // verification_rules — berversi, disetujui, dan dapat ditunjukkan
        // apa adanya kepada asesor. Batas flag pada alat tetap dihormati
        // sebagai pagar tambahan: pengaturan per-alat tidak boleh
        // dilonggarkan oleh aturan yang lebih permisif.
        // ----------------------------------------------------------------
        $keputusan = null;
        $bolehAuto = false;
        ...
        if ($autoVerify) {
            try {
                $keputusan = VerificationRuleService::putuskan([
                    'test_id'        => $testId,
                    'instrument_id'  => $instrumentId,
                    'flag'           => $flag,
                    'flag_alat'      => (string) ($r['flags'] ?? ''),
                    'nilai_num'      => $nilaiNum,
                    'delta_status'   => $delta['status'],
                    'delta_persen'   => $delta['persen'],
                    'is_kritis'      => $kritis,
                    // Umur yang tidak diketahui diperlakukan setara dengan
                    // satuan yang bentrok: nilainya ada, penilaiannya tidak
                    // dapat dipercaya, jadi tidak boleh lolos tanpa manusia.
                    'satuan_bentrok' => $satuanBentrok || $umurTidakPasti,
                    'qc_status'      => $qc['status'],
                    'lot_sah'        => $lot['sah'],
                    'lot_alasan'     => $lot['alasan'],
                    'jk'             => (string) ($konteks['jk'] ?? ''),
                    'umur_hari'      => $umur,
                ]);
            } catch (\Throwable $e) {
                Logger::error('Aturan autovalidasi tidak dapat dijalankan — hasil ditahan untuk verifikasi manual.', [
                    'galat' => $e->getMessage(),
                ]);
                $keputusan = ['validasi' => false, 'alasan' => 'Modul aturan verifikasi tidak dapat dimuat.'];
            }

            $bolehAuto = $keputusan['validasi']
                && self::flagDalamBatas($flag, $autoVerifyMaxFlag);

            if ($keputusan['validasi'] && !$bolehAuto) {
                $keputusan['validasi'] = false;
                $keputusan['alasan']   = sprintf(
                    'Aturan mengizinkan, tetapi batas flag alat "%s" lebih ketat.',
                    $autoVerifyMaxFlag
                );
            }
        }
        ...
        $data = [
            ...
            'instrument_id' => $instrumentId,
            // Ketertelusuran: run analitik, lot reagen, dan waktu
            // pemeriksaan MENURUT ALAT — bukan waktu LIS menerimanya.
            // Selisih keduanya bisa berjam-jam bila jalur sempat putus,
            // dan waktu alat itulah yang sah untuk perhitungan TAT.
            'run_id'         => $konteks['run_id'] ?? null,
            'reagent_lot_id' => $lotId,
            'analisis_at'    => self::waktuAnalisis($r),
            'satuan_ucum'    => $item['satuan_ucum'] ?? null,
            'message_id'    => $messageId,
            'is_manual'     => 0,
            'status'        => $status,
            ...
            // Flag kosong harus punya sebab yang tertulis. Tanpa ini,
            // verifikator melihat kolom kosong dan mengira sistem lupa
            // menilai — bukan bahwa sistem menolak menilai.
            'catatan'       => $umurTidakPasti
                ? 'Tanggal lahir pasien belum tercatat — rujukan menurut umur '
                  . 'tidak dapat dipilih, penilaian ditahan.'
                : null,
        ...
        // Jejak keputusan autovalidasi — termasuk saat hasilnya "tahan".
        // Justru penolakan yang paling sering ditanyakan kemudian.
        if ($keputusan !== null) {
            try {
                VerificationRuleService::catat($resultId, $keputusan);
            } catch (\Throwable $e) {
                Logger::error('Jejak keputusan autovalidasi gagal dicatat.', ['galat' => $e->getMessage()]);
            }
        }
        ...
        if ($kritis) {
            ...
            // Membuka antrean pelaporan berjangka waktu. Tanpa ini, nilai
            // kritis hanya menjadi baris log yang tidak menagih siapa pun.
            try {
                CriticalValueService::buka($resultId);
            } catch (\Throwable $e) {
                // Nilai kritisnya tetap tersimpan dan ber-flag kritis di
                // layar hasil; yang gagal hanya antrean pelaporannya.
                Logger::error('NILAI KRITIS tersimpan, tetapi antrean pelaporan gagal dibuka — laporkan manual.', [
                    'result_id' => $resultId,
                    'galat'     => $e->getMessage(),
                ]);
            }
        } else {
            // Hasil dikoreksi dan ternyata tidak lagi kritis: tagihan
            // pelaporan yang lama harus ditutup, bukan dibiarkan menganggu.
            try {
                CriticalValueService::batalkan($resultId, 'Hasil diperbarui dan tidak lagi kritis.');
            } catch (\Throwable $e) {
                Logger::warning('Antrean nilai kritis lama gagal ditutup.', ['result_id' => $resultId, 'galat' => $e->getMessage()]);
            }
        }

        if ($qc['menahan']) {
            Logger::warning('Hasil disimpan namun DITAHAN oleh kontrol kualitas', [
                'result_id' => $resultId,
                'test'      => $item['kode_test'],
                'qc'        => $qc['status'],
                'alasan'    => $qc['alasan'],
    ...
    /**
     * Waktu pemeriksaan menurut ALAT, bukan waktu LIS menerimanya.
     *
     * Analyzer mengirimkannya pada OBX-14 (HL7) atau field waktu pada
     * rekaman R (ASTM). Selisih keduanya biasanya beberapa detik, tetapi
     * dapat menjadi berjam-jam bila jalur komunikasi sempat putus dan
     * alat mengirim ulang antreannya. Untuk perhitungan TAT dan untuk
     * pertanyaan medikolegal "pukul berapa pemeriksaan ini dikerjakan",
     * waktu alat itulah yang sah.
     *
     * Nilai yang tidak masuk akal — kosong, gagal diurai, atau di masa
     * depan karena jam alat salah setel — dikembalikan sebagai null
     * daripada menyimpan tanggal yang menyesatkan.
     */
    private static function waktuAnalisis(array $r): ?string
    {
        $mentah = trim((string) ($r['completed_at'] ?? $r['observed_at'] ?? ''));
        if ($mentah === '') {
            return null;
        }
        ...
        // Jam alat yang salah setel adalah keluhan lapangan yang lazim.
        // Toleransi satu jam menampung beda zona waktu ringan; lebih dari
        // itu lebih baik dikosongkan daripada dipercaya.
        if ($ts > time() + 3600) {
            Logger::warning('Waktu pemeriksaan dari alat berada di masa depan — diabaikan', [
                'waktu_alat' => $mentah,
                'kode'       => $r['code'] ?? '',
            ]);
            return null;
     ...
     */
    /**
     * Pasang otomatis hasil menggantung milik order yang baru dibuat.
     *
     * Hanya hasil yang tertahan karena "sample ID tidak ditemukan" dan
     * yang sample ID-nya SAMA PERSIS dengan barcode tabung, nomor order,
     * nomor lab, atau nomor order Khanza milik order ini. Tidak ada
     * pencocokan kira-kira.
     *
     * Pagar pasien: bila alat membawa No. RM (PID-3) dan nomor itu BERBEDA
     * dari No. RM pasien order, hasil TIDAK dipasang. Barcode yang sama
     * pada dua pasien berbeda berarti ada yang salah tempel, dan itu harus
     * dilihat manusia — bukan diselesaikan diam-diam oleh mesin.
     *
     * Batas waktu 3 hari mencegah hasil lama ikut terpasang bila suatu
     * saat nomor dipakai ulang.
     *
     * @return array{terpasang:int, dilewati:list<string>}
     */
    public static function pasangOtomatisUntukOrder(int $orderId, ?int $userId = null, int $hariMaks = 3): array
    {
        $order = Database::selectOne(
            'SELECT o.id, o.no_order, o.no_lab, o.khanza_noorder, p.no_rm, p.khanza_no_rkm_medis
             FROM orders o JOIN patients p ON p.id = o.patient_id
             WHERE o.id = ? AND o.status <> \'cancelled\' LIMIT 1',
            [$orderId]
        );
        if ($order === null) {
            return ['terpasang' => 0, 'dilewati' => []];
        }

        $kunci = [];
        foreach (['no_order', 'no_lab', 'khanza_noorder'] as $k) {
            $v = trim((string) ($order[$k] ?? ''));
            if ($v !== '') {
                $kunci[] = $v;
            }
        }
        foreach (Database::select('SELECT barcode FROM specimens WHERE order_id = ?', [$orderId]) as $s) {
            $v = trim((string) $s['barcode']);
            if ($v !== '') {
                $kunci[] = $v;
            }
        }
        ...
        $tanda   = implode(',', array_fill(0, count($kunci), '?'));
        $orphans = Database::select(
            "SELECT id, sample_id, payload FROM orphan_results
             WHERE status = 'menunggu' AND alasan = 'sample_tidak_ditemukan'
               AND sample_id IN ($tanda)
               AND created_at >= DATE_SUB(NOW(), INTERVAL " . max(1, $hariMaks) . " DAY)
             ORDER BY id ASC",
            $kunci
        );

        $rmOrder = array_filter([
            trim((string) ($order['no_rm'] ?? '')),
            trim((string) ($order['khanza_no_rkm_medis'] ?? '')),
        ...
        foreach ($orphans as $o) {
            ...
            if ($rmAlat !== '' && $rmOrder !== [] && !in_array($rmAlat, $rmOrder, true)) {
                $dilewati[] = $o['sample_id'] . ': No. RM alat ' . $rmAlat . ' berbeda dengan pasien order';
                Logger::warning('Hasil menggantung TIDAK dipasang otomatis — No. RM berbeda.', [
            ...
            $r = self::pasangMenggantung((int) $o['id'], $orderId, $userId ?? 0, true);
            if ($r['sukses']) {
                $terpasang++;
                Logger::info('Hasil menggantung dipasang otomatis.', [
                    'sample_id' => $o['sample_id'],
                    'order'     => $order['no_order'],
                    'hasil'     => $r['tersimpan'] ?? 0,
                ]);
        ...
        return ['terpasang' => $terpasang, 'dilewati' => $dilewati];
    }

    public static function pasangMenggantung(int $orphanId, int $orderId, int $userId, bool $otomatis = false): array
    {
        ...
        Database::update('orphan_results', [
            'status'      => 'terpasang',
            'order_id'    => $orderId,
            // Pemasangan otomatis tidak dilakukan oleh petugas mana pun.
        ...
        Audit::log(
            ...
            'orphan_result',
            (string) $orphanId,
            ($otomatis ? 'Dipasang OTOMATIS ke order ' : 'Dipasang ke order ') . $order['no_order'] . ' (' . $tersimpan . ' hasil)'
        );
