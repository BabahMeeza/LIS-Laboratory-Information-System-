                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php elseif ($i['tipe_hasil'] === 'pilihan' && $i['opsi'] !== []): ?>
                                    <?php
                                    // Nilai lama dari alat bisa berejaan lain ("-", "+1").
                                    // Selaraskan agar pilihan yang benar langsung terpilih;
                                    // bila tetap tak cocok, tampilkan nilai aslinya sebagai
                                    // pilihan tambahan — jangan disembunyikan di balik
                                    // "— pilih —" seolah belum ada hasil.
                                    $nilaiAda  = trim((string) ($i['nilai'] ?? ''));
                                    try {
                                        $terpilih = \App\Services\PilihanHasil::cocokkan($nilaiAda, $i['opsi']);
                                    } catch (\Throwable $galatPilihan) {
                                        $terpilih = $nilaiAda;
                                    }
                                    $diLuarDaftar = $terpilih !== '' && !in_array($terpilih, $i['opsi'], true);
                                    ?>
                                    <select name="nilai[<?= (int) $i['order_item_id'] ?>]">
                                        <option value="">— pilih —</option>
                                        <?php if ($diLuarDaftar): ?>
                                            <option value="<?= $e($terpilih) ?>" selected><?= $e($terpilih) ?> (dari alat)</option>
                                        <?php endif; ?>
                                        <?php foreach ($i['opsi'] as $opsi): ?>
                                            <option value="<?= $e($opsi) ?>" <?= $terpilih === $opsi ? 'selected' : '' ?>><?= $e($opsi) ?></option>
                                        <?php endforeach; ?>
                                    </select>
