...
const a15 = require('./protocols/a15');
...
class Gateway {
  ...
  _tandaKoneksi(a) {
    return JSON.stringify([
      ...
      // Alat berbasis berkas: jalur folder ADALAH parameter koneksinya.
      // Tanpa ini, folder yang baru diisi di LIS tidak pernah terpakai
      // sampai middleware dijalankan ulang.
      a.file?.folderKeluar, a.file?.folderMasuk, a.file?.jedaMs,
    ]);
  }
  ...
  _pasang(alat) {
    ...
    switch (alat.transport) {
      ...
      case 'serial':
        ...
        break;
      // Alat yang bertukar lewat folder, mis. BioSystems A15. Tidak ada
      // soket sama sekali: hasil dibaca dari berkas ekspor, worklist
      // ditulis sebagai berkas impor.
      case 'file':
        transport = new FileTransport(alat, logger);
        break;
      default:
  ...
  _buatSesi(entri, koneksi) {
    ...
    // Protokol berkas BioSystems A15.
    //
    // Tidak ada sesi sungguhan: tiap potongan berkas langsung diurai dan
    // dikirim. Tidak ada ACK, tidak ada kerangka, tidak ada percakapan —
    // yang tiba adalah teks yang sudah utuh barisnya (transport berkas
    // menahan baris yang belum selesai ditulis).
    if (alat.protocol === 'a15') {
      const sesi = new raw.RawSession({ logger, nama: alat.code });

      sesi.on('text', (teks) => {
        entri.jumlahPesan += 1;
        const payload = a15.keNormal(teks, alat.code);

        if (payload.dilewati.length > 0) {
          // Baris yang tidak dikenali tidak boleh hilang diam-diam:
          // bentuk berkas yang berubah adalah cara paling senyap sebuah
          // integrasi berhenti bekerja.
          logger.warn(
            `[${alat.code}] ${payload.dilewati.length} baris tidak dikenali bentuknya `
            + `dan dilewati. Contoh: ${JSON.stringify(payload.dilewati[0])}`
          );
        }
  ...
  async tarikWorklist() {
    for (const entri of this.alat.values()) {
      ...
      // Alat berbasis berkas (A15): worklist ditulis sebagai import.txt,
      // bukan dikirim lewat sesi.
      if (alat.transport === 'file') {
        await this._worklistBerkas(entri);
        continue;
      }
  ...
  /**
   * Worklist untuk alat berbasis berkas.
   *
   * Urutannya sengaja: ambil TANPA menandai terkirim → tulis import.txt →
   * baru laporkan id yang benar-benar tertulis. Bila folder putus di
   * tengah jalan, antrian tetap "menunggu" dan dicoba lagi putaran
   * berikutnya, bukan hilang.
   */
  async _worklistBerkas(entri) {
    const alat = entri.alat;
    const transport = entri.transport;

    // Alasan penolakan dari A15 sendiri (Errors.txt), bila ada.
    for (const b of transport.bacaErrors()) {
      logger.warn(`[${alat.code}] A15 menolak worklist: ${b}`);
    }

    let antrian;
    try {
      antrian = await lis.ambilAntrianWorklist(alat.code, false);
    } catch (e) {
      logger.debug(`[${alat.code}] Pengambilan antrian worklist gagal: ${e.message}`);
      return;
    }
    if (!Array.isArray(antrian) || antrian.length === 0) return;
    ...
    for (const item of antrian) {
      const { isi: potong, ditolak } = a15.keImport([{
        sample_id: item.sample_id,
        tests: item.tests || [],
        urgent: item.priority === 'cito' || item.urgent === true,
      }]);

      for (const d of ditolak) {
        logger.warn(`[${alat.code}] Worklist ${d.sample_id}${d.test ? ' / ' + d.test : ''} ditolak: ${d.sebab}`);
      }

      if (potong === '') {
        // Tak satu pemeriksaan pun lolos batas A15 — jangan diulang terus.
        gagal.push(item.queue_id);
      } else {
        isi += potong;
        ack.push(item.queue_id);
      }
    }

    if (isi !== '' && !transport.tulisWorklist(isi)) {
      // Penulisan gagal: yang tadinya siap tetap "menunggu".
      ack.length = 0;
    }

    try {
      await lis.laporWorklist(alat.code, ack, gagal);
      if (ack.length > 0) {
        logger.info(`[${alat.code}] ${ack.length} sampel ditulis ke import.txt — operator menekan "Import session" di A15.`);
      }
    } catch (e) {
      // Berkas sudah tertulis tetapi LIS tidak menerima laporan: putaran
      // berikutnya akan menulis baris yang sama lagi. Lebih baik baris
      // ganda di import.txt (terlihat, dapat dihapus operator) daripada
      // worklist yang hilang tanpa jejak.
      logger.warn(`[${alat.code}] Laporan worklist ke LIS gagal: ${e.message}`);
    }
  }
