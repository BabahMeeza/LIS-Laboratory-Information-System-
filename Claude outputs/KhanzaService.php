...
final class KhanzaService
{
     ...
     */
    /**
     * Catat satu permintaan penarikan hasil oleh Khanza.
     *
     * Dipisahkan dari catat() karena arahnya masuk dan tidak ada respons
     * HTTP yang perlu disimpan — yang penting justru SEBAB bila hasilnya
     * kosong, supaya "ditarik gagal" tidak lagi harus ditebak.
     */
    public static function catatTarikan(string $noorder, array $ringkasan, ?string $sebab): void
    {
        try {
            Database::insert('khanza_sync_log', [
                'arah'      => 'masuk',
                'jenis'     => 'tarik_hasil',
                'ref_type'  => $noorder === '' ? null : 'order',
                'ref_id'    => $noorder === '' ? null : mb_substr($noorder, 0, 40),
                'endpoint'  => '/api/v1/khanza/hasil',
                'payload'   => mb_substr((string) json_encode($ringkasan, JSON_UNESCAPED_UNICODE), 0, 60000),
                'response'  => null,
                'http_code' => 200,
                'status'    => $sebab === null ? 'sukses' : 'gagal',
                'pesan'     => $sebab === null ? null : mb_substr($sebab, 0, 500),
            ]);
        } catch (\Throwable $e) {
            Logger::error('Gagal menulis jejak tarik hasil: ' . $e->getMessage());
        }
    }
