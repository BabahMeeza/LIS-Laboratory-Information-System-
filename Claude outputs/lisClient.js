...
class LisClient {
  ...
  /**
   * Laporkan hasil penulisan worklist per baris antrian.
   * Dipakai alat berbasis berkas: yang tertulis ditandai terkirim, yang
   * ditolak (mis. ID terlalu panjang) ditandai gagal agar tidak diulang
   * selamanya.
   */
  async laporWorklist(kodeAlat, ackIds = [], gagalIds = []) {
    if (ackIds.length === 0 && gagalIds.length === 0) return null;

    return this._minta(
      'GET',
      `/api/v1/instruments/${encodeURIComponent(kodeAlat)}/worklist` +
        `?ack_ids=${ackIds.join(',')}&gagal_ids=${gagalIds.join(',')}`
    );
  }
