    ...
    public function tat(): Response
    {
        ...
        $perTest = Database::select(
            "SELECT t.kode, t.nama, t.tat_menit AS target,
             ...
             HAVING jml > 0
             -- Ekspresi agregat ditulis ulang: MariaDB menolak alias
             -- agregat di dalam ekspresi ORDER BY (galat 1247).
             ORDER BY SUM(TIMESTAMPDIFF(MINUTE, o.tgl_order, r.verified_at) <= t.tat_menit) / COUNT(r.id) ASC
             LIMIT 40",
            [$dari, $sampai]
        );
    ...
    public function produktivitas(): Response
    {
        ...
        $perPetugas = Database::select(
            "SELECT u.nama, u.role,
             ...
             ORDER BY COALESCE(SUM(r.entered_by = u.id), 0) + COALESCE(SUM(r.verified_by = u.id), 0) DESC",
            [$dari, $sampai]
        );
